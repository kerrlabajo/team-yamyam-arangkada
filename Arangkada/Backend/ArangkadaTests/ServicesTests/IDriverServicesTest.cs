...
namespace ArangkadaTests.ServicesTests
{
    public class IDriverServicesTest
    {
        ...
        [Fact]
        public async void GetAll_HasDrivers_ReturnsDriverDtoList()
        {
            ...
            var expectedDriverDtoList = driverModelList.Select(driverModel =>
                new DriverDto
                {
                    Id = driverModel.Id,
                    OperatorName = driverModel.OperatorName,
                    VehicleAssigned = driverModel.VehicleAssigned,
                    FullName = driverModel.FullName,
                    Address = driverModel.Address,
                    ContactNumber = driverModel.ContactNumber,
                    LicenseNumber = driverModel.LicenseNumber,
                    ExpirationDate = driverModel.ExpirationDate,
                    DLCodes = driverModel.DLCodes
                }).ToList();

            _fakeDriverRepository.Setup(x => x.GetAll()).ReturnsAsync(driverModelList);
        ...
        {
            _fakeDriverRepository.Setup(x => x.GetAll())
                                 .Throws(new Exception("Simulated Exception"));
